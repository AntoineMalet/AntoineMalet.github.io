...
<details>
<summary>📈 Dérouler pour voir un exemple d'implémentation (R)</summary>
...
filtered_data <- reactive({
  ...
  filtered_df <- df %>% 
    dplyr::filter(abs(log2FC) >= input$log2FC_slider, pval <= input$p_val_slider)      # Apply plotly zoom filters if present   plotly_event <- event_data("plotly_relayout", priority = "event")   if (!is.null(plotly_event) && length(plotly_event) > 0) {     x_range_valid <- !is.null(plotly_event$`xaxis.range[0]`) && !is.null(plotly_event$`xaxis.range[1]`)
